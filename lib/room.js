...
const bmphrases = require('./utils/bmphrases')
...
class Room {
    ...
    guess(text, socket) {
        ...
        let feedback
        if(this.guessAllow) {         
            if(this.usersData[socket.nickname].matched == 'both') {
                feedback = "ТЫ СОВСЕМ КОНЧЕННЫЙ? ТЫ УЖЕ УГАДАЛ ВСЕ"
                socket.emit('bmphrase', feedback)
                return
            }
            ...
            this.io.sockets.in(this.name).emit('updateuserlist', {users:this.usersData, trackscount:this.trackscount})
            feedback = feedback || bmphrases('fail')
            socket.emit('bmphrase', feedback)
        
        } else {      
                feedback = 'ДОЖДИСЬ СЛЕДУЮЩЕЙ ПЕСНИ, ИДИОТ'
                socket.emit('bmphrase', feedback)
        }
    ...
    initiateCurrentTrack(response) {
        const songNumber = this.getRandomInt(1, config.maxRecentSong)
