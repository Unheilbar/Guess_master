...
class Room {
    ...
    guessAllow = false
    status = null // - 0 the song is currently playing/1 - loading the next song/2 - last track info sending/3 - game over
    ...
    updateStatus() {
        this.io.sockets.in(this.name).emit('statusupdate', this.status)
    }
    ...
    sendPlayTrack() {
        ...
        this.setRoomStatus(0)//Song is currently playing
        this.updateStatus()
    ...
    sendTrackInfo() {      
        ...
        if(this.trackscount<this.songsPerRound){
            this.setRoomStatus(1)//Loading next track
            this.updateStatus()
    ...
    gameOver() {
        this.setRoomStatus(3)//the game is over
        this.updateStatus()
